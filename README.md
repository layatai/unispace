...
The release script creates a universal Developer ID archive and a versioned notarized DMG such as `dist/UniSpace-1.0.7.dmg`, then validates code signing, stapling, and Gatekeeper acceptance. It never accepts credentials on the command line beyond the non-secret Keychain profile name.
