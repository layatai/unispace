...
final class FileTransferProtocolTests: XCTestCase {
    func testManifestValidatesRegularFilesAndComputesTotal() throws {
        let fixture = makeManifest(entries: [
            makeEntry(filename: "one.txt", byteCount: 10),
            makeEntry(filename: "two.pdf", byteCount: 25)
        ])

        XCTAssertNoThrow(try fixture.validated())
        XCTAssertEqual(fixture.totalByteCount, 35)
        XCTAssertEqual(fixture.displayName, "2 files")
    }

    func testManifestRejectsUnsafeAndDuplicateNames() {
        for name in ["", ".", "..", "../secret", "folder/file", "folder\\file", "bad\0name"] {
            let manifest = makeManifest(entries: [makeEntry(filename: name)])
            XCTAssertThrowsError(try manifest.validated(), "Expected \(name.debugDescription) to fail")
        }
    ...
    func testManifestEnforcesEntryAndTransferLimitsWithoutOverflow() {
    ...
    func testFrameModelsRoundTripThroughCodable() throws {
