...
final class FileTransferCoordinatorTests: XCTestCase {
    ...
    @MainActor
    func testDuplicateReceivedChunkIsAcknowledgedIdempotently() async throws {
        let fixture = Fixture()
        try await fixture.startAndConnect()
        let entry = TransferManifestEntry(
        ...
        XCTAssertEqual(await fixture.store.writeCount(), 1)
        await fixture.coordinator.stop()
    }
    ...
    @MainActor
    func testOneActiveOutgoingTransferPerPeer() async throws {
        let fixture = Fixture()
        try await fixture.startAndConnect()
...
private final class Fixture {
    ...
    func startAndConnect() async throws {
        ...
        while clock.now < deadline {
            if await coordinator.connectedDeviceIDs().contains(remote.id) { return }
            try? await Task.sleep(for: .milliseconds(20))
        }
