        ...
        return try condition.withLock {
            while true {
                ...
                guard condition.wait(until: deadline) else {
                    throw SimulationFailure("Timed out waiting for \(name)")
                }
            }
        }
    }
...
final class SimulationOrchestrator {
    ...
    func runAll() throws -> SimulationReport {
        ...
        try step("finder-multi-file") {
            ...
            guard sent.values["digests"] == received.values["digests"] else {
                throw SimulationFailure("Multi-file Finder digests do not match")
            }
        }
        ...
        try step("post-recovery-control") {
            ...
            guard recovered.visible.count > 0 else {
                throw SimulationFailure("No pointer input arrived after recovery")
            }
        }
    ...
    private func startWithRetry(
        ...
        throw lastError ?? SimulationFailure("\(name) could not start")
    }
    ...
    private func metrics(
        ...
        func decode(_ key: String) throws -> LatencySummary {
            guard let string = response.values[key], let data = string.data(using: .utf8) else {
                throw SimulationFailure("Missing \(key) metrics")
            }
    ...
    private func wait(
        ...
        throw SimulationFailure("Timed out waiting for \(description)")
    }
    ...
    private static func allocatePorts(excluding: Set<UInt16> = []) throws -> SimulationPorts {
        ...
        func next(socketType: Int32) throws -> UInt16 {
            ...
            throw SimulationFailure("Could not allocate unique loopback ports")
        }
    ...
    private static func availablePort(socketType: Int32) throws -> UInt16 {
        ...
        guard descriptor >= 0 else {
            throw SimulationFailure("Could not create port-allocation socket")
        }
        ...
        guard bound == 0 else {
            throw SimulationFailure("Could not bind port-allocation socket")
        }
        ...
        guard resolved == 0 else {
            throw SimulationFailure("Could not resolve allocated port")
        }
