...
public enum FileTransferFailureCode: String, Codable, Error, Equatable, Sendable {
...
public struct TransferManifest: Codable, Equatable, Sendable {
    ...
    public var displayName: String {
        entries.count == 1 ? entries[0].filename : "\(entries.count) files"
    }
    ...
    public func validated(limits: FileTransferLimits = .default) throws -> Self {
        ...
        for entry in entries {
            try entry.validated(limits: limits)
            ...
            guard !overflow, next <= limits.maximumTransferBytes else {
                throw FileTransferProtocolError.transferTooLarge(overflow ? UInt64.max : next)
            }
            total = next
        }
        return self
...
public struct TransferOffer: Codable, Equatable, Sendable {
    ...
    public init(manifest: TransferManifest) { self.manifest = manifest }
}
...
public struct TransferCompletion: Codable, Equatable, Sendable {
    public let transferID: TransferID
    public init(transferID: TransferID) { self.transferID = transferID }
}
