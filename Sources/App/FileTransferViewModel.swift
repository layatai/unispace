...
final class FileTransferViewModel: ObservableObject {
    ...
    private var configuredWorkspace: WorkspaceSnapshot?
    ...
    var effectiveDestinationID: DeviceID? {
        selectedDestinationID ?? candidateDevices.first(where: {
    ...
    func chooseFiles() {
        guard let destination = selectedDestinationID ?? effectiveDestinationID else {
    ...
    func send(_ urls: [URL], to destination: DeviceID? = nil) {
        let target = destination ?? selectedDestinationID ?? effectiveDestinationID
        Task {
            do {
    ...
    func deviceName(_ id: DeviceID) -> String {
        candidateDevices.first(where: { $0.id == id })?.name ?? "Mac"
    }
    ...
    private func refreshContext(from appModel: AppModel) async {
        ...
        if let selectedDestinationID,
           !candidateDevices.contains(where: { $0.id == selectedDestinationID }) {
            self.selectedDestinationID = nil
        }
        let inferredTarget = appModel.continuityTargetID
        if selectedDestinationID == nil, let inferredTarget {
            selectedDestinationID = inferredTarget
        }
        await coordinator.setAutomaticDestination(selectedDestinationID ?? inferredTarget)
        ...
        guard workspace != configuredWorkspace else { return }
