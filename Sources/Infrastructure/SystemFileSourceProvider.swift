...
public actor SystemFileSourceProvider: FileSourceProvider {
    ...
    public func prepare(
        ...
        for sourceURL in urls {
            ...
            guard values.isReadable != false else { throw FileSourceError.unreadable }
            guard let rawSize = values.fileSize, rawSize >= 0 else { throw FileSourceError.unreadable }
        ...
        let live = LiveSource(manifest: manifest, entries: liveEntries)
        sources[transferID] = live
    ...
    public func readChunk(
        ...
        guard let live = sources[transferID], let entry = live.entries[entryID] else {
    ...
    private func validate(_ entry: LiveEntry) throws {
        ...
        guard UInt64(values.fileSize ?? -1) == entry.byteCount,
    ...
    private func uniqueFilename(
        ...
        let extensionPart = (fallback as NSString).pathExtension
        let basePart = (fallback as NSString).deletingPathExtension
        var candidate = truncated(fallback, maximumBytes: maximumBytes)
        var index = 2
        while !usedNames.insert(folded(candidate)).inserted {
            let suffix = " \(index)"
            let suffixWithExtension = extensionPart.isEmpty ? suffix : suffix + "." + extensionPart
            let available = max(maximumBytes - suffixWithExtension.utf8.count, 1)
            candidate = truncated(basePart, maximumBytes: available) + suffixWithExtension
            index += 1
        }
        return candidate
    }
