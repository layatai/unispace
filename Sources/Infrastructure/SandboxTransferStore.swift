...
public actor SandboxTransferStore: TransferStore {
    ...
    public func prepareIncoming(
        ...
        let directory = transferDirectory(manifest.transferID)
        let metadataURL = metadataURL(manifest.transferID)
        ...
        for entry in manifest.entries {
            let partialURL = partialURL(transferID: manifest.transferID, entryID: entry.id)
            guard fileManager.createFile(atPath: partialURL.path, contents: Data()) else {
    ...
    public func write(
        ...
        _ = try chunk.validated(against: stored.manifest, limits: limits)
        ...
        let offsetIndex = try offsetIndex(chunk.entryID, in: stored)
        let durableOffset = stored.offsets[offsetIndex].offset
        ...
        stored.offsets[offsetIndex] = TransferEntryOffset(entryID: chunk.entryID, offset: nextOffset)
    ...
    public func recoverIncomingTransfers(
        ...
        for directory in directories {
            ...
            do {
                var stored = try load(transferID)
                _ = try stored.manifest.validated(limits: limits)
                var reconciledOffsets: [TransferEntryOffset] = []
                ...
                stored.offsets = reconciledOffsets
                stored.updatedAt = Date()
                try persist(stored)
    ...
    private func load(_ transferID: TransferID) throws -> StoredTransfer {
        ...
        return try JSONDecoder().decode(StoredTransfer.self, from: Data(contentsOf: url))
    }
    ...
    private func persist(_ stored: StoredTransfer) throws {
        ...
        let decoderCompatibleData = try encoder.encode(stored)
        try decoderCompatibleData.write(
            to: metadataURL(stored.manifest.transferID),
            options: [.atomic]
        )
    }
